...
export default async function DynamicPage({ params }: PageProps): Promise<any> {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <DummyUserAttr activityType={slug} />
      {/* Header Section */}
      ...
      <section className="py-16">
        <div className="container mx-auto px-6">
          <div className="space-y-6">
            {pageData.content_cards.flatMap(
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              (cardContainer: any, containerIndex: number) =>
                cardContainer.content_card.info_card.map(
                  // eslint-disable-next-line @typescript-eslint/no-explicit-any
                  (card: any, cardIndex: number) => (
                    <ContentCard
                      key={card.uid || `${containerIndex}-${cardIndex}`}
                      data={card}
                      name={card.name}
                      price={card.price}
